...
const dict: Record<string, { en: string; fr: string }> = {
  ...
  "learn.trainHint": { en: "The running model labels the traces, a small classifier is trained, then measured against the built-in detection on real traces it never saw. It is used only if it wins.", fr: "Le modèle en cours étiquette les traces, un petit classifieur est entraîné puis mesuré face à la détection intégrée sur des traces réelles jamais vues. Il n'est utilisé que s'il gagne." },
  ...
  "learn.gate.n": { en: "at least %n test rows", fr: "au moins %n lignes de test" },
