...
free_percent_policy="stop"
capture_max_swap_delta_bytes=${GALACTUS_GUARD_MAX_SWAP_DELTA_BYTES:-$((64 * 1024 * 1024))}
capacity_io_max_swapin_delta_bytes=$((4 * 16 * 1024))
max_output_bytes=${GALACTUS_GUARD_MAX_OUTPUT_BYTES:-2147483648}
min_volume_free_mb=${GALACTUS_GUARD_MIN_VOLUME_FREE_MB:-10240}
poll_seconds=0.20
max_wall_seconds=0
log_path=""
output_path=""
...
usage() {
    echo "usage: $0 [--min-free-percent N] [--max-footprint-gib N] [--max-preexisting-swap-mb N] [--swap-policy capture|benchmark|capacity-io] [--free-percent-policy stop|telemetry] [--poll-seconds N] [--max-wall-seconds N] --log FILE [--output FILE] [--swap-observation-log FILE --rss-inventory-log FILE] -- COMMAND..." >&2
    exit 2
}
...
if [[ "${free_percent_policy}" != "stop" && "${free_percent_policy}" != "telemetry" ]]; then
    usage
fi
if [[ "${free_percent_policy}" == "telemetry" && "${swap_policy}" == "capacity-io" ]]; then
    echo "error: capacity-io policy requires the free-percent stop" >&2
    exit 2
fi
...
free_percent_advisory_active=0
...
while kill -0 "${child_pid}" 2>/dev/null; do
    ...
    if [[ -n "${output_path}" && -f "${output_path}" ]]; then
        ...
        if ((output_bytes > max_output_bytes)); then
            stop_child "output_size_limit_exceeded"
            guard_breached=1
            break
        fi
    fi
    if [[ -n "${log_path}" ]]; then
        volume_free_mb="$(df -m "$(dirname "${log_path}")" 2>/dev/null | awk 'NR==2 {print $4}')"
        if [[ "${volume_free_mb}" =~ ^[0-9]+$ ]] && ((volume_free_mb < min_volume_free_mb)); then
            stop_child "output_volume_free_below_threshold"
            guard_breached=1
            break
        fi
    fi
    if ((free_percent < min_free_percent)); then
        if [[ "${free_percent_policy}" == "telemetry" ]]; then
            if ((free_percent_advisory_active == 0)); then
                append_event "free_percent_advisory_breach" "${child_pid}" "free_percent=${free_percent}<${min_free_percent}"
                free_percent_advisory_active=1
            fi
        else
    ...
    elif ((free_percent_advisory_active == 1)); then
        append_event "free_percent_advisory_cleared" "${child_pid}" "free_percent=${free_percent}>=${min_free_percent}"
        free_percent_advisory_active=0
    fi
