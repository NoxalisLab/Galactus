...
{
echo "=== differentiel llama4-scout couche ${COUCHE} — ${STAMP} (ngl ${GALACTUS_NGL:-99}) ==="
...
third_party/llama.cpp/build/bin/llama-perplexity "${COMMUN[@]}" --n-cpu-moe 99 \
    > "${OUT}/${STAMP}-llama4-stock.out" 2>&1 || true
grep -a "galactus_dump" "${OUT}/${STAMP}-llama4-stock.out" > "${OUT}/${STAMP}-llama4-stock.txt"
wc -l "${OUT}/${STAMP}-llama4-stock.txt"; grep -aoE "Final estimate: PPL = [0-9.]+" "${OUT}/${STAMP}-llama4-stock.out" | tail -1
echo ""
...
export GALACTUS_H4_CACHE_BYTES=33000000000
export GALACTUS_H4_QD=32
third_party/llama.cpp/build/bin/llama-perplexity "${COMMUN[@]}" --no-mmap --n-cpu-moe 99 \
    > "${OUT}/${STAMP}-llama4-cable.out" 2>&1 || true
grep -a "galactus_dump" "${OUT}/${STAMP}-llama4-cable.out" > "${OUT}/${STAMP}-llama4-cable.txt"
wc -l "${OUT}/${STAMP}-llama4-cable.txt"; grep -aoE "Final estimate: PPL = [0-9.]+" "${OUT}/${STAMP}-llama4-cable.out" | tail -1
echo ""
echo "=== PREMIERE DIVERGENCE ==="
diff <(grep -av "topk_galactus" "${OUT}/${STAMP}-llama4-stock.txt") \
     <(grep -av "topk_galactus" "${OUT}/${STAMP}-llama4-cable.txt") | head -14 || true
diff <(grep -av "topk_galactus" "${OUT}/${STAMP}-llama4-stock.txt") \
     <(grep -av "topk_galactus" "${OUT}/${STAMP}-llama4-cable.txt") > /dev/null \
  && echo "AUCUNE DIVERGENCE sur ${COUCHE}" || echo "(details dans les .txt)"
} 2>&1 | tee "${OUT}/${STAMP}-differentiel-llama4-${COUCHE}.log"
echo ""
