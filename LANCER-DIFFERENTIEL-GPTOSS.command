...
{
echo "=== differentiel gpt-oss couche ${COUCHE} — ${STAMP} (ngl ${GALACTUS_NGL:-99}) ==="
...
third_party/llama.cpp/build/bin/llama-perplexity "${COMMUN[@]}" --n-cpu-moe 99 \
    > "${OUT}/${STAMP}-goss-stock.out" 2>&1 || true
grep -a "galactus_dump" "${OUT}/${STAMP}-goss-stock.out" > "${OUT}/${STAMP}-goss-stock.txt"
wc -l "${OUT}/${STAMP}-goss-stock.txt"; grep -aoE "Final estimate: PPL = [0-9.]+" "${OUT}/${STAMP}-goss-stock.out" | tail -1
echo ""
...
third_party/llama.cpp/build/bin/llama-perplexity "${COMMUN[@]}" --no-mmap --n-cpu-moe 99 \
    > "${OUT}/${STAMP}-goss-cable.out" 2>&1 || true
grep -a "galactus_dump" "${OUT}/${STAMP}-goss-cable.out" > "${OUT}/${STAMP}-goss-cable.txt"
wc -l "${OUT}/${STAMP}-goss-cable.txt"; grep -aoE "Final estimate: PPL = [0-9.]+" "${OUT}/${STAMP}-goss-cable.out" | tail -1
echo ""
echo "=== PREMIERE DIVERGENCE ==="
diff <(grep -av "topk_galactus" "${OUT}/${STAMP}-goss-stock.txt") \
     <(grep -av "topk_galactus" "${OUT}/${STAMP}-goss-cable.txt") | head -14 || true
diff <(grep -av "topk_galactus" "${OUT}/${STAMP}-goss-stock.txt") \
     <(grep -av "topk_galactus" "${OUT}/${STAMP}-goss-cable.txt") > /dev/null \
  && echo "AUCUNE DIVERGENCE sur ${COUCHE}" || echo "(details dans les .txt)"
} 2>&1 | tee "${OUT}/${STAMP}-differentiel-goss-${COUCHE}.log"
echo ""
