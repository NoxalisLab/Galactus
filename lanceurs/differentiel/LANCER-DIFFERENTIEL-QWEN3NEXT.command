...
third_party/llama.cpp/build/bin/llama-perplexity "${COMMUN[@]}" --n-cpu-moe 99 \
    > "${OUT}/${STAMP}-qnext-stock.out" 2>&1 || true
grep -a "galactus_dump" "${OUT}/${STAMP}-qnext-stock.out" > "${OUT}/${STAMP}-qnext-stock.txt"
wc -l "${OUT}/${STAMP}-qnext-stock.txt"; grep -aoE "Final estimate: PPL = [0-9.]+" "${OUT}/${STAMP}-qnext-stock.out" | tail -1
echo ""
...
export GALACTUS_H4_CACHE_BYTES=16000000000
export GALACTUS_H4_QD=32
third_party/llama.cpp/build/bin/llama-perplexity "${COMMUN[@]}" --no-mmap --n-cpu-moe 99 \
    > "${OUT}/${STAMP}-qnext-cable.out" 2>&1 || true
grep -a "galactus_dump" "${OUT}/${STAMP}-qnext-cable.out" > "${OUT}/${STAMP}-qnext-cable.txt"
wc -l "${OUT}/${STAMP}-qnext-cable.txt"; grep -aoE "Final estimate: PPL = [0-9.]+" "${OUT}/${STAMP}-qnext-cable.out" | tail -1
echo ""
echo "=== PREMIERE DIVERGENCE ==="
diff <(grep -av "topk_galactus" "${OUT}/${STAMP}-qnext-stock.txt") \
     <(grep -av "topk_galactus" "${OUT}/${STAMP}-qnext-cable.txt") | head -14 || true
diff <(grep -av "topk_galactus" "${OUT}/${STAMP}-qnext-stock.txt") \
     <(grep -av "topk_galactus" "${OUT}/${STAMP}-qnext-cable.txt") > /dev/null \
  && echo "AUCUNE DIVERGENCE sur ${COUCHE}" || echo "(details dans les .txt)"
} 2>&1 | tee "${OUT}/${STAMP}-differentiel-qnext-${COUCHE}.log"
echo ""
