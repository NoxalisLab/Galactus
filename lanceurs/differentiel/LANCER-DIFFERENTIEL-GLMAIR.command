...
{
echo "=== differentiel glm-4.5-air couche ${COUCHE}, ${STAMP} (ngl ${GALACTUS_NGL:-99}) ==="
...
third_party/llama.cpp/build/bin/llama-perplexity "${COMMUN[@]}" --n-cpu-moe 99 \
    > "${OUT}/${STAMP}-glmair-stock.out" 2>&1 || true
grep -a "galactus_dump" "${OUT}/${STAMP}-glmair-stock.out" > "${OUT}/${STAMP}-glmair-stock.txt"
wc -l "${OUT}/${STAMP}-glmair-stock.txt"; grep -aoE "Final estimate: PPL = [0-9.]+" "${OUT}/${STAMP}-glmair-stock.out" | tail -1
echo ""
...
export GALACTUS_H4_CACHE_BYTES=35000000000
export GALACTUS_H4_QD=32
third_party/llama.cpp/build/bin/llama-perplexity "${COMMUN[@]}" --no-mmap --n-cpu-moe 99 \
    > "${OUT}/${STAMP}-glmair-cable.out" 2>&1 || true
grep -a "galactus_dump" "${OUT}/${STAMP}-glmair-cable.out" > "${OUT}/${STAMP}-glmair-cable.txt"
wc -l "${OUT}/${STAMP}-glmair-cable.txt"; grep -aoE "Final estimate: PPL = [0-9.]+" "${OUT}/${STAMP}-glmair-cable.out" | tail -1
echo ""
echo "=== PREMIERE DIVERGENCE ==="
diff <(grep -av "topk_galactus" "${OUT}/${STAMP}-glmair-stock.txt") \
     <(grep -av "topk_galactus" "${OUT}/${STAMP}-glmair-cable.txt") | head -14 || true
diff <(grep -av "topk_galactus" "${OUT}/${STAMP}-glmair-stock.txt") \
     <(grep -av "topk_galactus" "${OUT}/${STAMP}-glmair-cable.txt") > /dev/null \
  && echo "AUCUNE DIVERGENCE sur ${COUCHE}" || echo "(details dans les .txt)"
} 2>&1 | tee "${OUT}/${STAMP}-differentiel-glmair-${COUCHE}.log"
echo ""
