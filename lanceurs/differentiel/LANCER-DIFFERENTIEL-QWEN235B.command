...
{
echo "=== differentiel qwen3-235b couche ${COUCHE} — ${STAMP} (ngl ${GALACTUS_NGL:-99}) ==="
...
third_party/llama.cpp/build/bin/llama-perplexity "${COMMUN[@]}" --n-cpu-moe 99 \
    > "${OUT}/${STAMP}-qwen235-stock.out" 2>&1 || true
grep -a "galactus_dump" "${OUT}/${STAMP}-qwen235-stock.out" > "${OUT}/${STAMP}-qwen235-stock.txt"
wc -l "${OUT}/${STAMP}-qwen235-stock.txt"; grep -aoE "Final estimate: PPL = [0-9.]+" "${OUT}/${STAMP}-qwen235-stock.out" | tail -1
echo ""
...
export GALACTUS_H4_CACHE_BYTES=70000000000
export GALACTUS_H4_QD=32
third_party/llama.cpp/build/bin/llama-perplexity "${COMMUN[@]}" --no-mmap --n-cpu-moe 99 \
    > "${OUT}/${STAMP}-qwen235-cable.out" 2>&1 || true
grep -a "galactus_dump" "${OUT}/${STAMP}-qwen235-cable.out" > "${OUT}/${STAMP}-qwen235-cable.txt"
wc -l "${OUT}/${STAMP}-qwen235-cable.txt"; grep -aoE "Final estimate: PPL = [0-9.]+" "${OUT}/${STAMP}-qwen235-cable.out" | tail -1
echo ""
echo "=== PREMIERE DIVERGENCE ==="
diff <(grep -av "topk_galactus" "${OUT}/${STAMP}-qwen235-stock.txt") \
     <(grep -av "topk_galactus" "${OUT}/${STAMP}-qwen235-cable.txt") | head -14 || true
diff <(grep -av "topk_galactus" "${OUT}/${STAMP}-qwen235-stock.txt") \
     <(grep -av "topk_galactus" "${OUT}/${STAMP}-qwen235-cable.txt") > /dev/null \
  && echo "AUCUNE DIVERGENCE sur ${COUCHE}" || echo "(details dans les .txt)"
} 2>&1 | tee "${OUT}/${STAMP}-differentiel-qwen235-${COUCHE}.log"
echo ""
